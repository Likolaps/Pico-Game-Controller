...
bool SPOOF_KONAMI_CONTROLLER = false;
...
void joy_mode()
{
  if (tud_hid_ready())
  {
    ...
    for (int i = 0; i < ENC_GPIO_SIZE; i++)
    {
      ...
      delta *= 5; // Multipliez par 2 ou une autre valeur pour augmenter la sensibilité
      cur_enc_val[i] += delta;
...
void init()
{
  ...
  // Joy/KB Mode/konmai Switching
  uint8_t pressed_input = 255; // default
  ...
  for (int i = 0; i < SW_GPIO_SIZE; i++)
  {
    ...
    {
      pressed_input = i;
      break; // exit loop on first pressed button
    }
  }

  switch (pressed_input) // Replace '0' with the appropriate expression for your logic
  {
  case (0):
    loop_mode = &key_mode;
    ...
    break;
  case (1):
    SPOOF_KONAMI_CONTROLLER = true;
    loop_mode = &joy_mode;
    joy_mode_check = true;
    break;

  default:
    SPOOF_KONAMI_CONTROLLER = false;
    loop_mode = &joy_mode;
    joy_mode_check = true;
    break;
  }
